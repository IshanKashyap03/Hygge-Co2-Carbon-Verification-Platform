...
class CertificateDataClient:
    def __init__(self, data_process_callback: Callable, logger) -> None:
        self.client = mqtt.Client(
            ...
            clean_session=False,
            client_id=CERTIFICATE_DATA_BROKER_ID,
        )
        ...
        self.topic = CERTIFICATE_PUBLISHER_TOPIC
        self.logger.info("Certificate Data Client for API Initialized")
    ...
    def _on_message(self, client, userdata, message) -> None:
        ...
        transaction_hash = message_dict.get("transaction_hash")
        if self._validate_data("transaction_hash", transaction_hash):
            return

        trasaction_date = message_dict.get("transaction_date")
        try:
            trasaction_date = datetime.fromisoformat(trasaction_date)
        except ValueError:
            self.logger.critical("transaction_date is not in the correct format")
            return

        certificate_address = message_dict.get("certificate_address")
        if self._validate_data("certificate_address", certificate_address):
            return

        self.logger.success(
            "Received message: {certificate_id}, {amount}, {start_time}, {end_time}, {company_name}, {user_id}, {issue_date}, {transaction_hash}, {trasaction_date}, {certificate_address}",
    ...
    def _on_connect(self, client, userdata, flags, reason_code, properties) -> None:
        ...
        if reason_code == 0:
            ...
            client.subscribe(self.topic, qos=PAHO_QUALITY_OF_SERVICE_LEVEL_AT_MOST_ONCE)
        else:
            self.logger.error(
